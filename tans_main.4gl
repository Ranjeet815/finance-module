...
CONSTANT cBaseSQL = "SELECT customercode FROM customer"
CONSTANT cOrderBy = "customercode"



TYPE t_orders RECORD LIKE customer.*
TYPE t_order_num LIKE customer.customercode
TYPE t_stock_num LIKE customer.customercode

TYPE t_items RECORD
    
    suppliercode LIKE trans_details.suppliercode,
...
FUNCTION  open_trans()
    ...
    CALL STARTLOG("inputOrders.log")
    DATABASE testdb

    OPEN WINDOW inputForm WITH FORM "trans"
  --  CLOSE WINDOW SCREEN
...
PRIVATE FUNCTION fetchRecord(pFetchAction INTEGER) RETURNS BOOLEAN
    ...
    IF mCursorDefined == FALSE THEN
        DECLARE cursOrdersByNum CURSOR FROM "SELECT * FROM customer WHERE customercode = ?"
         DECLARE cursItemsByNum CURSOR FROM "SELECT * FROM trans_details WHERE customercode = ? ORDER BY suppliercode"
    ...
    TRY
        ...
        IF lReturnStatus THEN
            ...
            LET lIndex = 0
            FOREACH cursItemsByNum USING m_order.customercode INTO r_item.*
                LET lIndex = lIndex + 1
               
                LET mItemList[lIndex].suppliercode = r_item.suppliercode
        ...
        DISPLAY ARRAY mItemList TO trans_details.*
         ATTRIBUTES(ACCEPT=FALSE)
            ...
            ON ACTION LAST
                ...
                END IF
            ON ACTION add_rec
                IF inputOrder(inputMode.addMode) THEN
                    IF fetchRecord(fetchAction.refreshRecord) THEN
                        EXIT DISPLAY
                    END IF
                END IF
            ON ACTION edit_rec
                IF inputOrder(inputMode.changeMode) THEN
                    IF fetchRecord(fetchAction.refreshRecord) THEN
                        EXIT DISPLAY
                    END IF
                END IF
...
PUBLIC FUNCTION inputOrder(pMode INTEGER) RETURNS BOOLEAN
    DEFINE lReturnStatus BOOLEAN = FALSE
  ...
  -- name LIKE zoom_tr.training_dec
    ...
    ELSE
        ...
        LET r_order.customercode = 101
        LET r_order.customername= "N"
    END IF
     ...
     ATTRIBUTES(UNBUFFERED)
        ...
        INPUT r_order.* FROM U_User.*
         ATTRIBUTES(WITHOUT DEFAULTS=TRUE)
            BEFORE INPUT
                IF pMode == inputMode.changeMode THEN
                    CALL ui.Dialog.getCurrent().setFieldActive("U_User.customercode", FALSE)
                END IF
            AFTER FIELD customercode
                IF pMode == inputMode.addMode AND r_order.customercode IS NOT NULL THEN
                    SELECT customercode INTO lNumber
                      FROM trans_details WHERE customercode = r_order.customercode
        ...
        INPUT ARRAY lItemList FROM E_Event.*
         ATTRIBUTES(DELETE ROW=TRUE, INSERT ROW=FALSE, APPEND ROW=TRUE, AUTO APPEND=TRUE, WITHOUT DEFAULTS=TRUE)
            BEFORE ROW
                ...
                LET lItemList[lRow].suppliercode = NVL(lItemList[lRow].suppliercode, 0)
            ...
            BEFORE DELETE
                IF pMode == inputMode.changeMode THEN
                    ...
                    LET lDeletedItems[lIndex] = lItemList[lRow].suppliercode
                END IF
            AFTER ROW
                #Validate Data
               IF LENGTH(lItemList[lRow].suppliercode) == 0 THEN
                    ERROR "suppliercode "
      ...
      --  ON ACTION cencel
        ...
        AFTER DIALOG
            IF LENGTH(r_order.customername) == 0 THEN
                ERROR "customername is missing"
    ...
    ELSE
        TRY
            IF pMode == inputMode.addMode THEN
                INSERT INTO customer VALUES(r_order.*)
                CALL m_numList.appendElement()
                LET mCurrentRow = m_numList.getLength()
                LET m_numList[mCurrentRow] = r_order.customercode
                FOR lIndex = 1 TO lItemList.getLength()
                    INSERT INTO trans_details (customercode, suppliercode, trans_num, tran_date, acc_type, item_code,quantity,rate,ammount,ammount_paid,status)
                     VALUES(r_order.customercode, lItemList[lIndex].suppliercode,lItemList[lIndex].trans_num,lItemList[lIndex].tran_date,lItemList[lIndex].acc_type, lItemList[lIndex].item_code,  lItemList[lIndex].quantity,lItemList[lIndex].rate, lItemList[lIndex].ammount, lItemList[lIndex].ammount_paid, lItemList[lIndex].status)
                END FOR
            ELSE
                UPDATE customer
                   SET customer.*= r_order.*
                 WHERE customercode = r_order.customercode
                 FOR lIndex = 1 TO lDeletedItems.getLength()
                    DELETE FROM trans_details 
                     WHERE customer = r_order.customercode
                       AND suppliercode = lDeletedItems[lIndex]
                 END FOR
                 FOR lIndex = 1 TO lItemList.getLength()
                     SELECT customercode INTO lNumber FROM trans_details
                      WHERE customercode = r_order.customercode
                        AND suppliercode = lItemList[lIndex].suppliercode
